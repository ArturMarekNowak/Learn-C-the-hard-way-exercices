...
void copy(char to[], char from[])
{
	...
	//while loop will not enf if from isn't '\0' terminated
	while ((to[i] = from[i]) != '\0') {
		++i;
	}
}
...
{
	assert(from != NULL && to != NULL && "from and to can't ba NULL");
	int i = 0;
	int max = from_len > to_len - 1 ? to_len - 1 : from_len;

	//to_len must have at lease 1 byte
